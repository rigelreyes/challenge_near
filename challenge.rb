...
rsrc.snapshots.each do |snap|
  if "#{snap.description}" =~ /^backup_\d{4}(0?[1-9]|1[012])(0?[1-9]|[12][0-9]|3[01])([01]?\d|2[0-3])([0-5]?\d)([0-5]?\d)\[.{0,}\]$/
    year = Integer(snap.description[7..10])
    month = Integer(snap.description[11..12])
    day = Integer(snap.description[13..14])
    hour =  Integer(snap.description[15..16])
    minutes = Integer(snap.description[17..18])
    seconds = Integer(snap.description[19..20])
